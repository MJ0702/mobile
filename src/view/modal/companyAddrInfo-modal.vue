<template>
  <div>
    <Modal
      v-model='showAddModal'
      :closable='false'
      :mask-closable='false'
      :title='Title'
      width="680"
      :footer-hide='true'
    >
      <Form ref="formValidateAdd" :model="formValidateAdd" :label-width="60" class="top">
        <Row>
          <Col span="22">
            <FormItem label="公司" prop="name">
              <Select v-model="formValidateAdd.ucId" placeholder="请选择公司">
                <Option v-for="item in companyInfoList" :key="item.value" :value="item.id">{{ item.name }}</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="22">
            <FormItem label="省份" prop="province">
              <Input v-model="formValidateAdd.province" placeholder="请输入省份"></Input>
            </FormItem>
          </Col>
          <Col span="22">
            <FormItem label="城市" prop="city">
              <Input v-model="formValidateAdd.city" placeholder="请输入城市"></Input>
            </FormItem>
          </Col>
          <Col span="22">
            <FormItem label="地址" prop="addr">
              <Input v-model="formValidateAdd.addr" placeholder="请输入地址"></Input>
            </FormItem>
          </Col>
          <Col span="22">
            <FormItem label="状态" prop="isDefault">
              <Select v-model="formValidateAdd.isDefault" placeholder="请选择状态">
                <Option v-for="item in selList" :key="item.value" :value="item.value">{{ item.label }}</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="22" class="top">
            <FormItem label="备注">
              <Input v-model="formValidateAdd.memo" type="textarea" :rows="5" :autosize="{maxRows:5,minRows: 5}" placeholder="请输入备注"></Input>
            </FormItem>
          </Col>
          <Col span="23" class="top">
            <div class="rt">
              <Button type="text" @click="modalCancel">取消</Button>
              <Button type="primary" @click="modalOk">确定</Button>
            </div>
          </Col>
        </Row>
      </Form>
    </Modal>
  </div>
</template>
<script>
import { getCompanyInfoData } from '@/api/data'
export default {
  name: 'add-product-modal',
  // props: ['showModal', 'addTitle'],
  props: {
    content: Object
  },
  data () {
    return {
      showAddModal: this.content.addProductModal,
      Title: this.content.addTitle,
      isAdd: '',
      // 表单数据
      formValidateAdd: {
        ID: '',
        ucId: '',
        province: '',
        city: '',
        addr: '',
        idType: '',
        isDefault: '',
        memo: ''
      },
      companyInfoList: [],
      selList: [
        {
          value: '0',
          label: '默认'
        },
        {
          value: '1',
          label: '不默认'
        }
      ]
    }
  },
  methods: {
    // 文件上传成功时的钩子，返回字段为 response, file, fileList
    handleUploadicon (response) {
      this.formValidateAdd.imgUrl = response.imgUrl
      this.$Message.success('图片上传成功')
    },
    handleUpError () {
      this.$Message.error('图片上传失败')
    },
    // 文件格式验证失败时的钩子，返回字段为 file, fileList
    handleFormatError (file) {
      this.$Message.info('图片格式不正确,请上传正确的图片格式')
    },
    // 确定回调
    modalOk () {
      if (this.formValidateAdd.province === '') {
        this.$Message.error('请输入省份')
      } else if (this.formValidateAdd.city === '') {
        this.$Message.error('请输入城市')
      } else if (this.formValidateAdd.addr === '') {
        this.$Message.error('请输入地址')
      } else if (this.formValidateAdd.isDefault === '') {
        this.$Message.error('请选择状态')
      } else {
        let data = {}
        data.id = this.formValidateAdd.ID
        data.ucId = this.formValidateAdd.ucId
        data.province = this.formValidateAdd.province
        data.city = this.formValidateAdd.city
        data.addr = this.formValidateAdd.addr
        data.idType = '1'
        data.isDefault = this.formValidateAdd.isDefault
        data.memo = this.formValidateAdd.memo
        // 添加
        if (this.isAdd === 1) {
          data.url = '/address/add'
        // 编辑
        } else {
          data.url = '/address/update'
        }
        // 判断添加 / 编辑时提交数据给接口(调用父组件添加/编辑方法)
        this.$parent.getisAddData(data)
      }
    },
    // 取消回调
    modalCancel () {
      this.$emit('updateModelStatus', false)
    }
  },
  watch: {
    // 监听父组件传来的对象变换，用handler深监听
    content: {
      handler: function (newval, oldVal) {
        this.showAddModal = this.content.addProductModal
        this.Title = this.content.addTitle
        // 是否编辑
        this.isAdd = this.content.isEdit
        let edit = this.isAdd
        // 状态
        // 判断是否是编辑
        if (edit === 2) {
          this.formValidateAdd.ID = this.content.ID
          this.formValidateAdd.ucId = this.content.ucId
          this.formValidateAdd.name = this.content.name
          this.formValidateAdd.province = this.content.province
          this.formValidateAdd.city = this.content.city
          this.formValidateAdd.addr = this.content.addr
          this.formValidateAdd.memo = this.content.memo
          // 需要将status转换为字符串才可以给select赋值成功
          this.formValidateAdd.isDefault = this.content.isDefault
        } else {
          this.formValidateAdd.ID = ''
          this.formValidateAdd.ucId = ''
          this.formValidateAdd.name = ''
          this.formValidateAdd.province = ''
          this.formValidateAdd.city = ''
          this.formValidateAdd.addr = ''
          this.formValidateAdd.memo = ''
        }
      },
      deep: true
    }
  },
  mounted () {
    getCompanyInfoData().then(res => {
      // console.log(res.data.data)
      this.companyInfoList = res.data.data
    })
  }
}
</script>

<style lang="less">
.top {
  margin-top:15px;
}
.ivu-form .ivu-form-item-label {
  text-align: left;
}
.col-box-right{
  margin-right: 0px;
}
.col-box-left{
  margin-left: 5px;
}
.ivu-form-item-required .ivu-form-item-label:before{
  display: none;
}
.rt {
  float: right;
}
</style>
